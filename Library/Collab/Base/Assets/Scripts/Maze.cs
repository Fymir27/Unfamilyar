    ...
    Position[] Directions = new Position[]
{
    ...
    void Start()
    {
        ...
        for (int x = 10; x <= 13; x++)
        {
            wallsRoom3[North].Add(new Position(x, 13));
        }
        ...
        GameObject.Instantiate(PrefabMicrowave, Camera.main.ViewportToWorldPoint(GridToCameraPos(new Position(36, 35))), Quaternion.identity, setFurniture.transform); 

        //Bed
        finishedTiles[10, 25] = TileType.None;
        finishedTiles[10, 26] = TileType.None;
        GameObject.Instantiate(PrefabBed, Camera.main.ViewportToWorldPoint(GridToCameraPos(new Position(10, 25))) + new Vector3(0, 0.15f), Quaternion.identity, setFurniture.transform);

        //Bathtub
        finishedTiles[62, 29] = TileType.None;
        finishedTiles[62, 30] = TileType.None;
        GameObject.Instantiate(PrefabBathtub, Camera.main.ViewportToWorldPoint(GridToCameraPos(new Position(62, 29))) + new Vector3(0, 0.15f), Quaternion.identity, setFurniture.transform);

        //TV
        finishedTiles[6, 11] = TileType.None;
        GameObject.Instantiate(PrefabTV, Camera.main.ViewportToWorldPoint(GridToCameraPos(new Position(6, 11))), Quaternion.identity, setFurniture.transform);

        //Grammophone
        finishedTiles[32, 15] = TileType.None;
        GameObject.Instantiate(PrefabGrammophone, Camera.main.ViewportToWorldPoint(GridToCameraPos(new Position(32, 15))), Quaternion.identity, setFurniture.transform);
    ...
    public void PlaceFurniture()
    {
        ...
        for (int y = 1; y < Height * 2; y++)
        {
            for (int x = 1; x < Width * 2; x++)
            {
                ...
                if(finishedTiles[x + 1, y] == TileType.Furniture)
                {
                    possibleTypes.Add(FurnitureType.Horizontal);
                }

                if(finishedTiles[x, y + 1] == TileType.Furniture)
                {
